...
export default function DashboardPage() {
  ...
  async function handleCreateOrganization(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createOrganization(token, { name: orgName });
      ...
      <header className="border-b border-[#deddd4] bg-white px-6 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div>
            ...
            {user ? <p className="text-sm text-[#65655f]">Ola, {user.name}</p> : null}
          </div>
          <button
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm text-[#65655f]">
              Selecione ou crie a organizacao que representa sua operacao de campanha.
            </p>
            ...
            <div className="mt-6 space-y-3">
              {organizations.length === 0 ? (
                <div className="rounded-md border border-dashed border-[#d7d6cd] bg-white p-6 text-sm text-[#65655f]">
                  Nenhuma organizacao ainda. Crie a primeira para iniciar a operacao.
                </div>
              ) : (
                organizations.map((item) => (
                  <article key={item.membershipId} className="rounded-md border border-[#deddd4] bg-white p-4">
