...
### Status da 09.4 — CONCLUÍDA

Implementação registrada:

- **Adapter Evolution (Nest-free):** `packages/shared/src/evolution-send.client.ts` — `sendEvolutionText(input)` usa `fetch` nativo (sem dependência de `@nestjs/*`), reutilizável pelo Worker sem acoplamento a framework. Usa `EVOLUTION_API_URL` + `EVOLUTION_API_KEY` (header `apikey`) + `instanceName` (vindo de `ChannelAccount.externalAccountId`, resolvido pelo Worker). **Nunca** loga `apiKey`, destino/telefone ou conteúdo da mensagem; mensagens de erro são genéricas (não ecoam corpo bruto do provider). Nunca lança — sempre retorna `EvolutionSendResult` normalizado (`success` | `category`/`errorCode`/`errorMessage`/`ambiguous`) para o Worker decidir o próximo estado.
- **Categorização de falhas:** HTTP 429 → `PROVIDER_RATE_LIMIT`; 401/403 → `AUTHENTICATION_ERROR`; 404 → `INVALID_DESTINATION`; 400/422 → `CONTENT_REJECTED`; 5xx → `PROVIDER_UNAVAILABLE`; timeout/abort (sem resposta do provider) → `UNKNOWN_PROVIDER_STATE` com `ambiguous = true` (mensagem pode ter sido processada mesmo sem confirmação); erro de rede antes de qualquer resposta → `TRANSIENT_NETWORK`.
- **Modo piloto (default ligado, conservador):** `packages/shared/src/dispatch-feature-flags.ts` — `isDispatchPilotMode()` default **true**; `getDispatchPilotMaxItems()` default **5**; `getDispatchAllowedDestinations()` faz parse de CSV (`DISPATCH_ALLOWED_DESTINATIONS`, números normalizados) — allowlist vazia = sem restrição adicional de destino; `assertDispatchSendAllowed()` exige `DISPATCH_ENGINE_ENABLED` + `DISPATCH_QUEUE_ENABLED` + `DISPATCH_SEND_ENABLED` simultaneamente. `DISPATCH_SEND_ENABLED` mantém default **false** (nunca ativa automaticamente).
- **Retry com backoff:** `packages/shared/src/dispatch-send-retry.ts` — tabela fixa de backoff por `attemptCount`: 1min / 5min / 15min (`DISPATCH_SEND_RETRY_BACKOFF_MS`); `attemptCount` acima da tabela reutiliza o último valor (15min); `isDispatchRetryExhausted(attemptCount, maxAttempts)` decide quando ir para `FAILED` definitivo em vez de novo `RETRY_SCHEDULED`.
- **Migration `DispatchChannelUsageDaily`:** contador diário por instância (`dispatchChannelId` + `usageDate` únicos) com `sentCount`, `failedCount`, `lastSentAt`. Incrementado atomicamente (`upsert` com `increment`) a cada envio com sucesso; usado pelo Worker para decidir o delay mínimo entre envios do mesmo canal (baseado em `lastSentAt`).
- **`start` (claim `QUEUED → RUNNING`):** `POST /campaigns/:campaignId/dispatches/:dispatchId/start` (`DispatchStartService`, `apps/api/src/dispatches/dispatch-start.service.ts`). Exige OWNER/ADMIN (`requireApproveAccess`), `assertDispatchSendAllowed()` (ENGINE+QUEUE+SEND), Dispatch `QUEUED` com `queuedItems > 0`, `!requiringRedistribution` e volume dentro do teto do piloto (`assertDispatchStartWithinPilotLimit`, bloqueia se `totalItems > getDispatchPilotMaxItems()` quando `isDispatchPilotMode()`). Claim condicional via `updateMany` (`count !== 1` → `ConflictException`, corrida real de concorrência). Após o claim, percorre em lotes de 100 todos os items elegíveis (`QUEUED`/`RETRY_SCHEDULED`/`SCHEDULED`) e chama `DispatchSendProducer.ensureJob`/`requeueItem` para cada um — se existir job `completed`/`failed` no BullMQ (herdado da validação técnica 09.3), remove e recria com o **mesmo** `jobId` determinístico antes do envio real.
- **`canStart` (permissões/ações):** `buildDispatchAllowedActionsForPrepare` — exige OWNER/ADMIN, Dispatch `QUEUED`, `queuedItems > 0`, `!requiringRedistribution`, `DISPATCH_ENGINE_ENABLED` + `DISPATCH_QUEUE_ENABLED` + `DISPATCH_SEND_ENABLED` e `isDispatchStartWithinPilotLimit(totalItems)`.
- **Worker (`apps/worker/src/dispatch-send.processor.ts`) — decisão de modo:** o path técnico da 09.3 (`runTechnicalValidation`, inalterado) é usado sempre que o Dispatch **não** está `RUNNING` **ou** está `RUNNING` com `DISPATCH_SEND_ENABLED=false` (mantém apenas `TECHNICAL_VALIDATED`, nunca chama a Evolution). O envio real (`runRealSend`) só ocorre quando `Dispatch.status === RUNNING && DISPATCH_SEND_ENABLED=true`.
- **Envio real — claim e last-mile:** reclama `QUEUED`/`SCHEDULED`/`RETRY_SCHEDULED` → `PROCESSING` (lock atômico, mesmo padrão da 09.3). Antes de chamar o adapter: revalida `Contact.status` (DELETED/BLOCKED → `SKIPPED`) e `OptOut` ativo (canal `WHATSAPP` ou nulo → `SKIPPED`); revalida `normalizedDestination` (regex de dígitos, 10–15) → `FAILED` se inválido; bloqueia destino fora da allowlist do piloto (`isDispatchDestinationAllowed`) → `SKIPPED` sem chamar a Evolution.
- **Seleção/rotação/pausa de canal:** reaproveita o failover da 09.3 (`selectNextEligibleDispatchChannel` + `buildReassignmentUpdate`); aplica rotação por volume (`rotateEveryMessages`, `shouldRotateChannel`) quando o canal atual não sofreu failover; respeita a janela operacional (mesma lógica da 09.3); calcula o delay mínimo entre envios do mesmo canal (aleatório entre `minDelaySeconds`/`maxDelaySeconds`, mais pausa de lote/pausa longa quando `sentItems` cruza `batchSize`/`longPauseEveryMessages`) comparando com `DispatchChannelUsageDaily.lastSentAt`; se o tempo não decorreu, o item volta a `SCHEDULED` e o job é adiado (`moveToDelayed`) — `DEFERRED_CHANNEL_DELAY`.
- **Resultados após a chamada Evolution:** sucesso → `SENT` (`providerMessageId`, `providerStatus`, `sentAt`, `attemptCount++`, canal `sentItems++`/`consecutiveErrors = 0`, `DispatchChannelUsageDaily` incrementado); falha transitória (`TRANSIENT_NETWORK`/`PROVIDER_UNAVAILABLE`/`PROVIDER_TIMEOUT`) → `RETRY_SCHEDULED` com `nextRetryAt` (backoff 1/5/15min) até esgotar `maxAttempts`, depois `FAILED`; falha permanente (`INVALID_DESTINATION`/`CONTENT_REJECTED`/demais) → `FAILED` direto; falha ambígua (`UNKNOWN_PROVIDER_STATE`, timeout/abort) → status `UNKNOWN_PROVIDER_STATE`, **sem** retry automático (`nextRetryAt = null`), para revisão humana.
- **403/429 → cooldown + failover:** quando a política permite (`protectionPolicy.pauseOn403`/`pauseOn429`, default `true`), incrementa `DispatchChannel.consecutiveErrors`, define `cooldownUntil` (5min × tentativas, capado em 30min) e `operationalStatus = COOLDOWN`; tenta imediatamente outro canal elegível (`excludeCurrent`) e reagenda o item (`SCHEDULED` + `moveToDelayed` em ~5s); sem canal disponível, apenas mantém o cooldown e reagenda no mesmo canal.
- **Progresso do Dispatch:** após cada item resolvido, `recomputeDispatchProgress` reconta todos os status via `groupBy` e atualiza os contadores do Dispatch (`pendingItems`, `queuedItems` — agrega `QUEUED`+`SCHEDULED`+`RETRY_SCHEDULED`, `processingItems`, `sentItems`, `failedItems` — agrega `FAILED`+`UNKNOWN_PROVIDER_STATE`, `skippedItems`, `canceledItems`); quando não resta nenhum item pendente/enfileirado/processando, marca `COMPLETED` (sem erros) ou `COMPLETED_WITH_ERRORS` (`completedAt = now`).
- **Rotas/Web:** `POST .../dispatches/:dispatchId/start`; botão "Iniciar envio" no detalhe do Dispatch (com confirmação explícita e badge "Modo piloto"), mensagens de status para `QUEUED`/`RUNNING`/`COMPLETED`/`COMPLETED_WITH_ERRORS`; `getDispatchProgressSteps` corrigido para refletir corretamente as etapas Fila/Execução/Conclusão conforme o status real.
- **Testes:** `dispatch-feature-flags.spec.ts`, `dispatch-send-retry.spec.ts`, `evolution-send.client.spec.ts` (shared); `dispatch-start.util.spec.ts`, `dispatch-start.service.spec.ts`, `dispatch-prepare.util.spec.ts` (API — permissões, `SEND=false` bloqueia `start`, limite do piloto, conflito de concorrência real); `dispatch-send.processor.spec.ts` (Worker — sucesso `SENT`, opt-out/bloqueio, destino inválido, allowlist do piloto, failover, cooldown 429, `UNKNOWN_PROVIDER_STATE`, retry transitório, duplicado sem reenvio); `dispatches.progress.spec.ts` (Web).
- **Fora de escopo (mantido para 09.5+):** pausar/retomar/cancelar execução em `RUNNING`, parada emergencial, homologação final de limites (09.8).
...
### Corre��o obrigat�ria 09.6.2 � Fechamento e homologa��o das blindagens

**Status: implementada (c�digo + testes). Homologa��o operacional pendente.**

Princ�pio: nenhuma regra aparece como ativa sem status honesto:
...
- Sem garantia anti-ban; multim�dia fora de escopo; `DISPATCH_SEND_ENABLED=false`.
